...
resource "docker_image" "streamlit_app_docker_image" {
  ...
  build {
    context  = "${path.cwd}/../streamlit-app"
    platform = "linux/amd64"
  }

  triggers = {
    dir_sha = sha1(join("", [for f in fileset("${path.cwd}/../streamlit-app", "**") : filesha1("${path.cwd}/../streamlit-app/${f}")]))
  }
  force_remove = true
}
...
resource "aws_ecr_repository" "query_structured_data_lambda_repository" {
  name                 = "query_structured_data_lambda"
  image_tag_mutability = "MUTABLE"
